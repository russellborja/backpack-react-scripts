> *This project was bootstrapped with [Create React App][1] + [backpack-react-scripts][2].*
  It is very similar to one that you would create using [Create React App][1] without
  [backpack-react-scripts][2], so please refer to [it's documentation][3] for more usage information.
...
`backpack-react-scripts` is listed as a `devDependency` in your [package.json][4] and exposes the following
npm scripts:
...
- `npm run lint`: Lints all javascript using [`eslint-config-skyscanner`][5].
...
```json
{
  ...
  "backpack-react-scripts": {
    "babelIncludeRegex": "/node_modules[\\\/]my-module-prefix-/"
  }
}
```

The above example assumes that the module you want to compile is named with the prefix `my-module-prefix-`.

## Upgrading `backpack-react-scripts`

The [Backpack squad][6] provide ongoing maintenance and bugfixes to `backpack-react-scripts`. Please refer to
[it's changelog][7] for upgrade guides.
...
the [Backpack squad][6] - we are open to contributions.
...
**Note: this is a one-way operation. Once you eject, you can’t go back and you won't get support from the Backpack
squad!**
...
[2]: http://git.prod.skyscanner.local/backpack/create-react-app/tree/master/packages/react-scripts
[3]: https://github.com/facebookincubator/create-react-app/blob/master/packages/react-scripts/template/README.md
[4]: ./package.json
[5]: https://www.npmjs.com/package/eslint-config-skyscanner
[6]: https://skyscanner.slack.com/messages/backpack/
[7]: http://git.prod.skyscanner.local/backpack/create-react-app/blob/master/packages/react-scripts/CHANGELOG.md
